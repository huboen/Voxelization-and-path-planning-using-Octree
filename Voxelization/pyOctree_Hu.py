...
class Octree:
    ...
    def visualize(self, stl_path=None,boundingboxes=None,octree=True):
        ...
        #     merged_line_set.points.extend(line_set.points)
        #     num_points = len(line_set.points)
        #     lines = [[line[0] + num_points * i, line[1] + num_points * i] for i, line in enumerate(line_set.lines)]
        #     merged_line_set.lines.extend(lines)
    ...
    def __create_lines__(self, node, line_set_list):
        if node.depth != 3 and node.children:
            for child_node in node.children:
